...
def main(args,loadpath):
    ...
    model_glob.train() 
    # 加载攻击模型
    model_glob.load_state_dict(torch.load(loadpath))
    ...
    for user in range(args.num_users):
        ...
        for key in model_glob.state_dict().keys():
            ...
            model_local_dict['fc3.weight'] = torch.from_numpy(np.load('./save/heads/'+str(args.frac)+'/'+str(args.embed_dim)+'/'+str(args.epochs)+'/'+ args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
                args.shard_per_user) +'_'+str(user)+'_weight.npy'))
            model_local_dict['fc3.bias'] = torch.from_numpy(np.load('./save/heads/'+str(args.frac)+'/'+str(args.embed_dim)+'/'+str(args.epochs)+'/'+args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
                args.shard_per_user) +'_'+str(user)+'_bias.npy'))       
    ...
    for iter in range(args.epochs + 1):
        ...
        for ind, idx in enumerate(idxs_users):
            ...
            np.save('./save_fine/heads/'+str(args.frac)+'/'+str(args.embed_dim)+'/'+ str(args.epochs)+ '/' +args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
    ...
    accs_dir = './save_fine/accs/' + str(args.epochs)+'/'
    accs_csv = './save_fine/accs/'+str(args.epochs)+'/accs_' + args.dataset + '_' + str(args.num_users) + '_' +  str(
    ...
    # 和上面一样保存到csv文件中
    if args.use_watermark:
        if not os.path.exists('./save_fine/water_acc/'):
            os.makedirs('./save_fine/water_acc/')
        all_detect_all_dir = './save_fine/water_acc/all_detect_all_rate_' + args.alg + '_' + args.dataset + '_'+str(args.num_users) + '_' + str(
...
def plot_accs(args,embed_dims,epochs=100):
    ...
    for dim in embed_dims:
        accs_csv = './save_fine/accs/'+str(epochs)+'/accs_' + args.dataset + '_' + str(args.num_users) + '_' +  str(
    ...
    #保存图片
    if not os.path.exists('./save_fine/pics'):
        os.makedirs('./save_fine/pics')
    plt.savefig('./save_fine/pics/accs of different embed_dim.png')
...
def plot_heatmap(args,embed_dims,epochs=100):
    ...
    for dim in embed_dims:
       ...
       if not os.path.exists('./save_fine/pics'):
            os.makedirs('./save_fine/pics')
       plt.savefig('./save_fine/pics/heatmap of {} in 100 epochs.png'.format(dim))
...
if __name__ == '__main__':
   ...
   args.epochs = 100 # 微调的轮数
   for emd in emds:
        args.embed_dim = emd     
        # 这里的use_watermark是为了加载模型名字
        # 实际上fine-tune的时候是不需要use_watermark的
        ...
        model_save_path = "./save/glob_models/" + str(args.frac)+'/'+str(args.embed_dim)+'/accs_' + args.alg + '_' + args.dataset + '_' + str(args.num_users) + '_' + str(
                args.shard_per_user) + '_iter'+ str(args.use_watermark) +'100.pt'
        main(args,loadpath=model_save_path)
    
   plot_accs(args,emds)
   plot_heatmap(args,emds)
